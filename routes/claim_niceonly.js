...
async function get_table_lock(t) {
  return t.none(
    ...
    'LOCK TABLE SearchFieldsNiceonly IN SHARE ROW EXCLUSIVE MODE;'
  )
}
...
async function assign_expired(
  ...
  return t.oneOrNone(
    ...
    WHERE id = ( \
      SELECT id FROM SearchFieldsNiceonly \
      WHERE \
        base = ${base} AND \
...
async function assign_first(
  ...
  return t.oneOrNone(
    "INSERT INTO SearchFieldsNiceonly ( \
      base, \
      search_start, \
      search_end, \
      search_range, \
      ...
      ${username} \
    WHERE NOT EXISTS ( \
      SELECT 1 FROM SearchFieldsNiceonly \
      WHERE base = ${base} \
    ) RETURNING *;",
    {
      base: base,
...
router.get('/', async function (req, res, next) {
  ...
  db.tx(async (t) => {
    ...
    if (req.query.base) {
      ...
      if (base_data.status_niceonly > 1) {
